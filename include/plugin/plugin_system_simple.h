// Simplified plugin system without OpenCV dependency
#pragma once
...
// Simple image structure to replace OpenCV
struct SimpleImage {
    int width;
    int height;
    int channels;
...
struct SimpleRect {
    ...
    SimpleRect(int _x, int _y, int _w, int _h) 
...
public:
    ...
    // Feature extraction
    FeatureDescriptor extractFeatures(const SimpleImage& image, const SimpleRect& bbox) const;
...
private:
    ...
    std::vector<float> computeRGBHistogram(const SimpleImage& roi) const;
    std::vector<float> computeHSVHistogram(const SimpleImage& roi) const;
    std::vector<float> computeEdgeHistogram(const SimpleImage& roi) const;
    std::vector<float> computeShapeFeatures(const SimpleImage& roi) const;
    std::vector<float> computeTextureFeatures(const SimpleImage& roi) const;
    
    // JSON parsing
    PluginMetadata parseMetadata(const std::string& jsonPath) const;
    
    // Negative sample mining
    std::vector<SimpleRect> generateNegativeSamples(const SimpleImage& image, 
                                                  const SimpleRect& positiveBbox,
                                                  int numSamples = 10) const;
    
    // Image loading helper
    SimpleImage loadImage(const std::string& path) const;
    SimpleImage extractROI(const SimpleImage& image, const SimpleRect& rect) const;
};
...
private:
    struct PluginHead {
        std::string className;
        std::unique_ptr<Conv2D> convLayer;
        int outputChannels;
    };
